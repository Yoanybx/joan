...
The current Rust implementation includes an executable language preview: `.joan` syntax, AST, static type/effect checks, linear one-shot authority slots, deterministic bytecode, standalone non-executing bytecode verification, a bounded VM and structured receipts. Native code generation, general linear values and delegation, JOAN Lattice transport, a distributed network, marketplace and real external effects remain outside this release.
