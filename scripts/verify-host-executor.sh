...
"$joan" digest-v1 joan.host-execution-request.v1 "$work/domain-probe" >"$work/rust-domain.json"
node tools/jce1-reference.mjs digest joan.host-execution-request.v1 "$work/domain-probe" >"$work/js-domain.json"
...
assert(failed.schema === "joan.host-execution-receipt.v0", "failure receipt schema drift");
...
assert(failed.receipt_digest.domain === "joan.host-execution-receipt.v1", "receipt identity drift");
NODE

printf '%s\n' 'JOAN host executor gate passed: process separation, dependency isolation, bounded failure receipts and CLI compatibility.'
